...
export default function OrganizationIdScriptLayout() {
  ...
  const resolver = zodResolver(customEventCreateFormSchema)
  const form = useForm<z.infer<typeof customEventCreateFormSchema>>({
    resolver,
    defaultValues: {
      name: '',
      description: '',
  ...
  const onSubmit = (data: z.infer<typeof customEventCreateFormSchema>) => {
    fetcher.submit(
      {
        name: data.name,
      ...
      {
        action: `/organizations/${organization.id}/custom-events`,
        method: 'POST',
  ...
  return (
    <div className="flex flex-1 flex-col gap-2 overflow-hidden">
      <div className="flex justify-between">
        ...
        <Link
          to={`/organizations/${organization.id}/custom-events`}
          className={cn(
            'flex gap-2',
