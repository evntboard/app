...
func (app *application) recursiveSort(nodes []*Node) []*Node {
	...
	for _, node := range nodes {
		if node.Type == TypeFolder || node.Type == TypeTrigger {
			node.Children = app.recursiveSort(node.Children)
		}
	}
...
func (app *application) generateTree(path string, triggerRecords []*models.Record, conditionsRecords []*models.Record, sharedRecords []*models.Record) *Node {
	rootName := "root"
	...
	return &Node{
		...
		Enable:   root.Enable,
		IsLeaf:   root.IsLeaf,
		Children: app.recursiveSort(root.Children),
	}
}
