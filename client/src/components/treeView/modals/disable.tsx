...
export const DisableModal = ({organizationId, entity, action, onClose}: Props) => {
  ...
  const handleOk = async () => {
    ...
    try {
      switch (entity.type) {
        case "shared": {
          await ky.get(`/api/organization/${organizationId}/shared/${entity?.id}/disable`)
          break;
        }
        case "trigger": {
          await ky.get(`/api/organization/${organizationId}/trigger/${entity?.id}/disable`)
          break;
        }
        case "folder":
          await ky.get(`/api/organization/${organizationId}/tree/disable?path=${entity.slug}`)
          break;
      }
      onClose()
    } catch (e) {
